...
function filterItems(items) {
  ...
  return items.filter((it) => {
    ...
    const title = normalizeTitle(it.title || "").toLowerCase();
...
function renderHistory() {
  const host = $("history");
  if (!host) return;
  const rawItems = getHistoryItems();
  let items = rawItems;
  let changed = false;
  if (items.length) {
    items = items.map((it) => {
      const fixedTitle = normalizeTitle(it.title || "");
      if (fixedTitle && fixedTitle !== it.title) changed = true;
      return fixedTitle ? { ...it, title: fixedTitle } : it;
    });
    if (changed) saveHistoryItems(items);
  }
  ...
  host.innerHTML = filtered
    .map((it, idx) => {
      ...
      const title = normalizeTitle(it.title || "Món gợi ý");
